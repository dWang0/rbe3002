...
def aStar(rmap, start, end):# example call aStar(testmap, Node(4,0,46), Node(2,0,100))
    ...
    print(calculateHeuristic(startGrid, end))
    error = 0
    done = 0
    if(rmap[start.x][start.y]==100 or rmap[end.x][end.y] == 100):
        error = 1
    turns = [0, 45, -45, 90, -90, 135, -135,]
    while(done == 0 and error == 0):
        ...
        neighborNodes =[]
        for turn in turns:
            newTheta = (currentNode.node.theta+turn+360)%360
            if(newTheta <= 22.5 or newTheta >= 337.5):
                dx=1
                dy=0
            elif(newTheta <= 67.5 and newTheta >= 22.5):
                dx=1
                dy=1
            elif(newTheta <= 112.5 and newTheta >= 67.5):
                dx=0
                dy=1
            elif(newTheta <= 157.5 and newTheta >= 112.5):
                dx=-1
                dy=1
            elif(newTheta <= 202.5 and newTheta >= 157.5):
                dx=-1
                dy=0
            elif(newTheta <= 247.5 and newTheta >= 202.5):
                dx=-1
                dy=-1
            elif(newTheta <= 292.5 and newTheta >= 247.5):
                dx=0
                dy=-1
            else:
                dx=1
...
def addFrontier(frontierList, frontier):
    ...
    for f in frontierList:
        if ((frontier.cost.f < f.cost.f) and insert == -1):
            insert = i
        elif ((frontier.cost.f == f.cost.f) and (frontier.cost.g < f.cost.g) and insert == -1):
            insert = i
...
def angleToGrid(angle):
    if(abs(angle) <= 22.5 or abs(angle) >= 337.5):
        return 0
    elif(abs(angle) <= 67.5 and abs(angle) >= 22.5):
        return 45
    elif(abs(angle) <= 112.5 and abs(angle) >= 67.5):
        return  90
    elif(abs(angle) <= 157.5 and abs(angle) >= 112.5):
        return 135
    elif(abs(angle) <= 202.5 and abs(angle) >= 157.5):
        return 180
    elif(abs(angle) <= 247.5 and abs(angle) >= 202.5):
        return  225
    elif(abs(angle) <= 292.5 and abs(angle) >= 247.5):
        return 270
    else:
        return 315
def calculateHeuristic(node, goal):
    ...
    if(dx < 0):
        theta += 180
    theta = angleToGrid((theta + 360)%360)
...
if __name__ == '__main__':
    ...
    grid =aStar(listToMapXY(rvizmap, 37, 37), Node(1,1,46), Node(8,15,0))#))
    #printPath(grid)
